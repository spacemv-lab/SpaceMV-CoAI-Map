 ...
 * OpenSeaMap 数据同步服务配置
 */
export interface OpenSeaMapSyncServiceConfig {
  sourceName: string;
  datasetName: string;
  externalId: string;
  tag: string;
 ...
 * OpenSeaMap 数据同步服务
 *
 * 定时从 OpenSeaMap 获取全球船舶位置数据
 */
@Injectable()
export class OpenSeaMapSyncService extends ExternalDataSyncService implements OnModuleInit {
  protected readonly logger = new Logger(OpenSeaMapSyncService.name);
  ...
  constructor(
    ...
    gisQueue: GisQueue,
    private openSeaMapAdapter: OpenSeaMapAdapter,
    private minioService: MinioService,
    serviceConfig: OpenSeaMapSyncServiceConfig,
  ) {
    ...
    this.config = {
      ...
      sourceType: 'SYNC',
      defaultTags: ['ais', 'maritime', 'open-source', serviceConfig.tag],
      externalId: serviceConfig.externalId,
    };
  }

  async onModuleInit() {
    this.logger.log(`OpenSeaMapSyncService[${this.config.sourceName}] 初始化完成`);
  }
  ...
  async fetchData(): Promise<FetchedData> {
    return this.openSeaMapAdapter.fetchAll();
  }
  ...
  async sync(): Promise<SyncResult> {
    ...
    try {
      ...
      // 6. 更新 DatasetVersion 的 filePath 和 fileSize
      if (snapshotKey) {
  ...
  /**
   * 创建 OpenSeaMap 同步服务实例
   */
  static createInstance(
    datasetService: DatasetService,
    gisQueue: GisQueue,
    minioService: MinioService,
    config: OpenSeaMapSyncServiceConfig,
  ): OpenSeaMapSyncService {
    const adapter = new OpenSeaMapAdapter();
    return new OpenSeaMapSyncService(datasetService, gisQueue, adapter, minioService, config);
  }
}
