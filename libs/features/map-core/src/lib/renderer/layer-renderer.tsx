...
export function LayerRenderer() {
  ...
  const updatePointLayerStyle = async (
    ...
    const firstFeatureGeometry = layer.data?.features?.[0]?.geometry;
    const isPointLayer =
      layer.geometryType?.toUpperCase() === 'POINT' ||
      (layer.data?.features?.length &&
        firstFeatureGeometry &&
        typeof firstFeatureGeometry === 'object' &&
        'type' in firstFeatureGeometry &&
        (firstFeatureGeometry as { type?: string }).type?.toUpperCase() === 'POINT');
  ...
  useEffect(() => {
    return () => {
      // Destroy all point renderers with try-catch for safety
      getPointRendererEntries().forEach(([, renderer]) => {
        try {
          renderer.destroy();
        } catch (e) {
          console.warn('[LayerRenderer] Failed to destroy point renderer:', e);
        }
      });
      clearPointRendererRegistry();
    };
