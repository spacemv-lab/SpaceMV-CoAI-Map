...
import {
  ...
  GraduatedColorsPanel,
} from './controls';
import { RenderingType, GraduatedConfig } from '../../types/graduated-style';
...
export function StylePanel() {
  ...
  // Get current rendering type from layer style
  const renderingType = useMemo(() => {
    if (!layer) return 'simple';
    return layer.style.renderingType || 'simple';
  }, [layer]);

  // Get graduated config from layer style
  const graduatedConfig = useMemo(() => {
    if (!layer) return undefined;
    return layer.style.graduatedConfig;
  }, [layer]);

  // Handle rendering type change
  const handleRenderingTypeChange = (type: RenderingType) => {
    if (!layer) return;

    if (type === 'simple') {
      // Switch to simple: clear graduated config, keep base style
      updateLayerStyle(layer.id, {
        renderingType: 'simple',
        graduatedConfig: undefined,
      });
    } else {
      // Switch to graduated: set default config
      updateLayerStyle(layer.id, {
        renderingType: 'graduated',
        graduatedConfig: {
          field: '',
  ...
  // Handle graduated config change
  const handleGraduatedConfigChange = (newConfig: GraduatedConfig) => {
    if (!layer) return;
    updateLayerStyle(layer.id, {
      graduatedConfig: newConfig,
    });
  };
  ...
  return (
    <div className="bg-transparent p-4 text-sm pointer-events-auto h-full overflow-y-auto">
      ...
      {/* Conditional Content based on Rendering Type */}
      {renderingType === 'simple' ? (
        ...
        /* Graduated Colors Section */
        <div className="border-t pt-4 border-gray-200">
          <h4 className="font-medium text-gray-700 mb-3">分级色彩配置</h4>
          <GraduatedColorsPanel
            layer={layer}
            config={graduatedConfig}
            onChange={handleGraduatedConfigChange}
          />
        </div>
      )}
    </div>
