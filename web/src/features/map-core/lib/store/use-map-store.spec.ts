...
describe('useMapStore - Project Persistence', () => {
  beforeEach(() => {
    useMapStore.setState({
  ...
  it('should capture a serializable project state snapshot', () => {
    ...
    useMapStore.getState().setBasemap('tianditu-img');

    const snapshot = useMapStore.getState().captureProjectState();
    ...
    expect(snapshot.basemap).toBe('tianditu-img');
    expect(snapshot.layers).toHaveLength(1);
    expect(snapshot.layers[0].id).toBe('layer-1');
    expect(snapshot.layers[0].type).toBe('dataset');
    expect(snapshot.layers[0].datasetId).toBe('dataset-123');
    expect((snapshot.layers[0] as Record<string, unknown>).sourceId).toBeUndefined();
    expect((snapshot.layers[0] as Record<string, unknown>).dataSource).toBeUndefined();
  });

  it('should preserve draw layers as draw when capturing a project state', () => {
    useMapStore.getState().addLayer({
      id: 'draw-layer',
      name: 'Draw Layer',
      type: 'Draw',
      ...
      data: {
        type: 'FeatureCollection',
        features: [{ id: 'f1', properties: { name: 'test' }, geometry: null as never }],
      },
    });
    ...
    expect(snapshot.layers).toHaveLength(1);
    expect(snapshot.layers[0].type).toBe('draw');
    expect(snapshot.layers[0].data).toEqual({
      type: 'FeatureCollection',
      features: [{ id: 'f1', properties: { name: 'test' }, geometry: null }],
    });
  });
});
